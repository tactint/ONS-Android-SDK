...
import com.ons.android.ONSOptOutResultListener;
...
import com.ons.android.util.MetaDataUtils;

/**
 * ONS's Opt Out Module.
 *
 */
@Module
@Singleton
public class OptOutModule extends ONSModule {
    ...
    private static final String OPT_OUT_PREFERENCES_NAME = "com.ons.optout";

    private static final String OPTED_OUT_FROM_BATCHSDK_KEY = "app.ons.opted_out";

    private static final String SHOULD_SEND_OPTIN_EVENT_KEY = "app.ons.send_optin_event";
        ...
        if (isOptedOut == null) {
            ...
            if (prefs.contains(OPTED_OUT_FROM_BATCHSDK_KEY)) {
                ...
                if (isOptedOut) {
                    Logger.info(
                        TAG,
                        "ONS has been set to be Opted Out from by default in your app's manifest. You will need to call ONS.optIn() before performing anything else."
                    );
                }
            }
        }
    ...
    public Promise<Void> optOut(
        ...
        final ONSOptOutResultListener listener
    ) {
        ...
        if (!oldOptOutValue) {
            ...
            eventPromise.catchException(e ->
                new Handler(context.getMainLooper())
                    .post(() -> {
                        if (listener != null) {
                            if (listener.onError() == ONSOptOutResultListener.ErrorPolicy.CANCEL) {
