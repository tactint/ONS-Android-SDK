...
public enum ONSMigration {
     ...
     */
    @NonNull
    public static EnumSet<ONSMigration> fromValue(int value) {
        EnumSet<ONSMigration> migrations = EnumSet.noneOf(ONSMigration.class);
        for (ONSMigration migration : values()) {
            if (migration != NONE && (value & migration.value) == migration.value) {
                migrations.add(migration);
            }
        }
        if (migrations.isEmpty()) {
            migrations.add(ONSMigration.NONE);
        }
        return migrations;
    }
     ...
     */
    public static int toValue(@NonNull EnumSet<ONSMigration> migrations) {
        int val = 0;
        for (ONSMigration migration : migrations) {
            val |= migration.value;
        }
        return val;
    }
    ...
    public static boolean isCustomIDMigrationDisabled(@Nullable Integer value) {
        return (value != null && (fromValue(value).contains(ONSMigration.CUSTOM_ID)));
    }
    ...
    public static boolean isCustomDataMigrationDisabled(@Nullable Integer value) {
        return (value != null && (fromValue(value).contains(ONSMigration.CUSTOM_DATA)));
    }
}
