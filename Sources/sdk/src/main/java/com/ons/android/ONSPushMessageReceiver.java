...
/**
 * ONS's implementation of FCM's Push BroadcastReceiver
 */
@PublicSDK
public class ONSPushMessageReceiver extends WakefulBroadcastReceiver {

    private static final String TAG = "ONSPushMessageReceiver";
    ...
    private boolean scheduleJob(@NonNull Context context, @NonNull Intent intent) {
        try {
            ...
            jobExtras.putBundle(ONSPushJobService.JOB_EXTRA_PUSH_DATA_KEY, intentExtras);
            int jobId = (int) (Math.random() * Integer.MAX_VALUE);
            JobInfo job = new JobInfo.Builder(jobId, new ComponentName(context, ONSPushJobService.class))
            ...
            Logger.internal(TAG, "Could schedule ONS push presentation job", e1);
        }
        return false;
    }

    private void startPresentationService(@NonNull Context context, @NonNull Intent intent) {
        // Explicitly specify that ONSPushService will handle the intent and start it.
        ComponentName comp = new ComponentName(context.getPackageName(), ONSPushService.class.getName());
