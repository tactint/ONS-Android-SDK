...
/**
 * Convenience object to retrieve standardized ONS data out of a ONS Push intent.<br/>
...
@PublicSDK
public class ONSPushPayload implements PushUserActionSource {

    /**
     * Represents a payload parsing exception.
     * This usually means that the given bundle or receiver intent doesn't contain data that ONSPushPayload understands.
     */
    @PublicSDK
    ...
    ONSPushPayload(@NonNull Bundle extras) throws ParsingException {
        internalPushData = InternalPushData.getPushDataForReceiverBundle(extras);

        if (internalPushData == null) {
            throw new ParsingException("Payload does not contain required ONS push data");
        }

        rawData = new Bundle(extras);
    }

    ONSPushPayload(@NonNull RemoteMessage message) throws ParsingException {
        rawData = ONSPushHelper.firebaseMessageToReceiverBundle(message);
    ...
    /**
     * Attempt to extract the ONS Push data contained within the specified Bundle.<br/>
     * This is usually the method you want to use when reading this payload from an activity opened by ONS,
     * or when you wrote an existing ONSPushPayload to a Bundle/Intent extras using {@link #writeToBundle(Bundle)} / {@link #writeToIntentExtras(Intent)}.
     *
     * @param bundle Bundle containing ONS Push data. Parsed data is read from the key defined by {@link ONS.Push#PAYLOAD_KEY}.
     * @return A ONSPushPayload instance.
     * @throws ParsingException Thrown if the argument doesn't contain valid ONS Push data
     */
    public static ONSPushPayload payloadFromBundle(@Nullable Bundle bundle) throws ParsingException {
        //noinspection ConstantConditions
        if (bundle == null) {
            throw new IllegalArgumentException("Extras cannot be null");
        }

        final Bundle payload = bundle.getBundle(ONS.Push.PAYLOAD_KEY);

        if (payload == null) {
            throw new ParsingException("Given bundle does not contain push information in ONS.Push.PAYLOAD_KEY");
        }

        return new ONSPushPayload(payload);
    }

    /**
     * Attempt to extract the ONS Push data contained within the specified Intent.<br/>
     ...
     * If you wrote an existing ONSPushPayload to a Bundle/Intent extras using {@link #writeToBundle(Bundle)} / {@link #writeToIntentExtras(Intent)}, you should rather use {@link #payloadFromBundle(Bundle)}
     *
     * @param intent Broadcast receiver intent containing ONS Push data.
     * @return A ONSPushPayload instance.
     * @throws ParsingException Thrown if the argument doesn't contain valid ONS Push data
     */
    public static ONSPushPayload payloadFromReceiverIntent(@NonNull Intent intent) throws ParsingException {
        //noinspection ConstantConditions
        if (intent == null) {
        ...
        return new ONSPushPayload(extras);
    }

    /**
     * Attempt to extract the ONS Push data contained within the specified Intent extras.<br/>
     * This is usually the method you want to use when reading this payload from a push broadcast receiver/service, NOT from an activity intent.
     * If you wrote an existing ONSPushPayload to a Bundle/Intent extras using {@link #writeToBundle(Bundle)} / {@link #writeToIntentExtras(Intent)}, you should rather use {@link #payloadFromBundle(Bundle)}
     *
     * @param extras Broadcast receiver intent's extras containing ONS Push data.
     * @return A ONSPushPayload instance.
     * @throws ParsingException Thrown if the argument doesn't contain valid ONS Push data
     */
    public static ONSPushPayload payloadFromReceiverExtras(@NonNull Bundle extras) throws ParsingException {
        //noinspection ConstantConditions
        if (extras == null) {
            throw new IllegalArgumentException("Extras cannot be null");
        }
    ...
    /**
     * Attempt to extract the ONS Push data contained within the specified Firebase RemoteMessage.<br/>
     ...
     * If you wrote an existing ONSPushPayload to a Bundle/Intent extras using {@link #writeToBundle(Bundle)} / {@link #writeToIntentExtras(Intent)}, you should rather use {@link #payloadFromBundle(Bundle)}
     *
     * @param remoteMessage Firebase remote message containing ONS Push data.
     * @return A ONSPushPayload instance.
     * @throws ParsingException Thrown if the argument doesn't contain valid ONS Push data
     */
    public static ONSPushPayload payloadFromFirebaseMessage(@NonNull RemoteMessage remoteMessage)
        ...
        return new ONSPushPayload(remoteMessage);
    }
    ...
    public void writeToBundle(@NonNull Bundle bundle) {
        ...
        bundle.putBundle(ONS.Push.PAYLOAD_KEY, rawData);
    }
    ...
    public void writeToIntentExtras(@NonNull Intent intent) {
        ...
        intent.putExtra(ONS.Push.PAYLOAD_KEY, rawData);
    }
     ...
     */
    public ONSMessage getLandingMessage() {
        final JSONObject landingMessage = internalPushData.getLandingMessage();
        if (landingMessage == null) {
            return null;
        }
        return new ONSLandingMessage(rawData, landingMessage);
    }

    /**
     * Get the actions associated with this push, if any.
     */
    public List<ONSNotificationAction> getActions() {
        List<ONSNotificationAction> actions = internalPushData.getActions();
