...
package com.ons.android.messaging.view;
...
public class FlexboxLayout extends ViewGroup {
    ...
    @IntDef(
        { ALIGN_ITEMS_FLEX_START, ALIGN_ITEMS_FLEX_END, ALIGN_ITEMS_CENTER, ALIGN_ITEMS_BASELINE, ALIGN_ITEMS_STRETCH }
    )
    @Retention(RetentionPolicy.SOURCE)
    ...
    private void measureHorizontal(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // loop)
        {
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                if (
                    lp.flexBasisPercent != LayoutParams.FLEX_BASIS_PERCENT_DEFAULT && widthMode == MeasureSpec.EXACTLY
                ) {
                ...
                largestHeightInRow =
                    Math.max(largestHeightInRow, child.getMeasuredHeight() + lp.topMargin + lp.bottomMargin);
            ...
            for (FlexLine flexLine : mFlexLines) {
                ...
                for (int i = viewIndex; i < viewIndex + flexLine.itemCount; i++) {
                    ...
                    if (mFlexWrap != FLEX_WRAP_WRAP_REVERSE) {
                        ...
                        largestHeightInLine =
                            Math.max(largestHeightInLine, child.getHeight() + marginTop + lp.bottomMargin);
                    } else {
                        ...
                        largestHeightInLine =
                            Math.max(largestHeightInLine, child.getHeight() + lp.topMargin + marginBottom);
                    }
                }
    ...
    private void measureVertical(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            largestWidthInColumn =
                Math.max(largestWidthInColumn, child.getMeasuredWidth() + lp.leftMargin + lp.rightMargin);
    ...
    private void determineMainSize(@FlexDirection int flexDirection, int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (FlexLine flexLine : mFlexLines) {
            if (flexLine.mainSize < mainSize) {
                childIndex = expandFlexItems(flexLine, flexDirection, mainSize, paddingAlongMainAxis, childIndex);
            } else {
                childIndex = shrinkFlexItems(flexLine, flexDirection, mainSize, paddingAlongMainAxis, childIndex);
            }
        }
    }
        ...
        if (alignItems == ALIGN_ITEMS_STRETCH) {
            ...
            for (FlexLine flexLine : mFlexLines) {
                for (int i = 0; i < flexLine.itemCount; i++, viewIndex++) {
                    ...
                    LayoutParams lp = (LayoutParams) view.getLayoutParams();
                    if (
                        lp.alignSelf != LayoutParams.ALIGN_SELF_AUTO && lp.alignSelf != LayoutParams.ALIGN_SELF_STRETCH
                    ) {
                        continue;
                    }
                    switch (flexDirection) {
    ...
    private void setMeasuredDimensionForFlex(
        ...
        switch (widthMode) {
            ...
            case MeasureSpec.UNSPECIFIED:
                {
                    widthSizeAndState =
                        ViewCompat.resolveSizeAndState(calculatedMaxWidth, widthMeasureSpec, childState);
                    break;
                }
            default:
        ...
        switch (heightMode) {
            ...
            case MeasureSpec.UNSPECIFIED:
                {
                    heightSizeAndState =
                        ViewCompat.resolveSizeAndState(calculatedMaxHeight, heightMeasureSpec, childState);
                    break;
                }
            default:
    ...
    private boolean isWrapRequired(
        ...
        return (mode == MeasureSpec.EXACTLY || mode == MeasureSpec.AT_MOST) && maxSize < currentLength + childLength;
    }
