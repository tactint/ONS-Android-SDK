...
import com.ons.android.ONSDisplayReceiptJobService;
...
import com.ons.android.displayreceipt.CacheHelper;
import com.ons.android.displayreceipt.DisplayReceipt;
...
@Module
@Singleton
public class DisplayReceiptModule extends ONSModule {
    ...
    public void onsDidStart() {
        ...
        if (Boolean.TRUE.equals(optOutModule.isOptedOutSync(context))) {
            Logger.internal(TAG, "ONS is opted out, refusing to send display receipt.");
            return;
        }
        ...
        if (dmi >= 0) {
            try {
                ...
                JobInfo.Builder builder = new JobInfo.Builder(
                    jobId,
                    new ComponentName(context, ONSDisplayReceiptJobService.class)
                )
                ...
                Logger.internal(TAG, "Could not schedule ONS display receipt job", e1);
            }
        } else {
