...
public final class IntentParser {
     ...
     */
    private static final String ALREADY_SHOWN_LANDING_KEY = "com.ons.messaging.push.shown";

    /**
     * Key used to remember if an intent open has been tracked already
     */
    private static final String ALREADY_TRACKED_OPEN_KEY = "com.ons.open.tracked";

    /**
     * Key used to store if an intent comes from a push notification
     */
    private static final String FROM_PUSH_KEY = "com.ons.from_push";
     ...
     * Can't really ever be removed, as many devs rely on it
     */
    private static final String PUSH_ID_KEY = "com.ons.push_id";
     ...
     * Parsed payload
     */
    private ONSPushPayload payload = null;
    ...
    public IntentParser(Intent intent) {
        ...
        try {
            ...
            if (extras != null) {
                final Bundle payload = extras.getBundle(ONS.Push.PAYLOAD_KEY);

                if (payload != null) {
                    this.payload = new ONSPushPayload(payload);
                }
            }
        } catch (Exception e) {
            Logger.internal(TAG, "Unexpected error while parsing ONSPushPayload from intent", e);
        }
    }
    ...
    /**
     * Does this intent contains a ONS Messaging landing message
     *
     * @return true if it does
     */
     ...
     *
     * @return ONS Message to display
     */
    @Nullable
    public ONSMessage getLanding() {
        if (payload == null) {
    ...
    /**
     * Add push extras to an intent, to be picked up and tracked by ONS later
     */
    public static void putPushExtrasToIntent(Bundle fullPayload, InternalPushData onsData, Intent launchIntent) {
        ...
        if (fullPayload != null && !fullPayload.isEmpty()) {
            launchIntent.putExtra(ONS.Push.PAYLOAD_KEY, fullPayload);
        }

        final String pushId = onsData.getPushId();
        if (pushId != null) {
    ...
    /**
     * Copy all of ONS's possible internal extras from an intent to another
     */
    ...
    /**
     * Copy all of ONS's possible internal extras from a bundle
     */
    public static void copyExtras(@Nullable Bundle from, @Nullable Bundle to) {
        ...
        if (from.containsKey(ONS.Push.PAYLOAD_KEY)) {
            Bundle payload = from.getBundle(ONS.Push.PAYLOAD_KEY);
            if (payload != null) {
                to.putBundle(ONS.Push.PAYLOAD_KEY, payload);
            }
        }
