package com.ons.android.push.formats;
...
public class APENFormat extends BaseFormat implements NotificationFormat {
    ...
    public RemoteViews generateCollapsedView(@NonNull String packageName) {
        final RemoteViews view = new RemoteViews(packageName, R.layout.com_onssdk_notification_layout_apen_collapsed);
        view.setTextViewText(R.id.com_onssdk_notification_title, title);
        view.setTextViewText(R.id.com_onssdk_notification_body, body);

        Bitmap finalIcon = icon != null ? icon : picture;
        if (finalIcon != null) {
            view.setImageViewBitmap(R.id.com_onssdk_notification_icon1, finalIcon);
        } else {
            view.setViewVisibility(R.id.com_onssdk_notification_icon1, View.GONE);
        }
        return view;
    }
    ...
    public RemoteViews generateExpandedView(@NonNull String packageName) {
        ...
        if (layoutType.shouldForceLayoutHeight()) {
            view = new RemoteViews(packageName, R.layout.com_onssdk_notification_layout_apen_expanded_200dp);
        } else {
            view = new RemoteViews(packageName, R.layout.com_onssdk_notification_layout_apen_expanded);
        }

        view.setTextViewText(R.id.com_onssdk_notification_title, title);
        view.setTextViewText(R.id.com_onssdk_notification_body, body);

        view.setViewVisibility(R.id.com_onssdk_notification_icon_centercrop, View.GONE);
        view.setViewVisibility(R.id.com_onssdk_notification_icon_fitcenter, View.GONE);

        int targetImageResID;
        if (layoutType.shouldFitCenter()) {
            targetImageResID = R.id.com_onssdk_notification_icon_fitcenter;
        } else {
            targetImageResID = R.id.com_onssdk_notification_icon_centercrop;
        }
        if (picture != null) {
