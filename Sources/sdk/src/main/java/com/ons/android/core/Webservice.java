...
import com.ons.android.core.URLBuilder.CryptorMode;
import com.ons.android.core.Webservice.WebserviceError.Reason;
...
import com.ons.android.di.providers.SecureDateProviderProvider;
...
import com.ons.android.post.PostDataProvider;
...
public abstract class Webservice {
    ...
    protected Webservice(Context context, RequestType type, String urlPattern, String... parameters)
        ...
        this.isDowngradedCipher = ParametersProvider.get(applicationContext)
                .get(ParameterKeys.WS_CIPHERV2_LAST_FAILURE_KEY) != null;
    }
     ...
     *
     * @param parameters
     * @return the same parameters with ONS key prepended
     */
    protected static String[] addONSApiKey(String[] parameters) {
    ...
    protected void addDefaultHeaders() {
        ...
        headers.put("X-ONS-Nonce", this.id);

        String cipherVersion = "2";
        if (isDowngradedCipher) {
            headers.put("X-ONS-Cipher-Downgraded", "1");
            cipherVersion = "1";
        }

        // Add response body cipher version
        headers.put("X-ONS-Accept-Cipher", cipherVersion);
        if (type == RequestType.POST) {
            // Add request body cipher version
            headers.put("X-ONS-Content-Cipher", cipherVersion);
        }
    }
     ...
     *
     * @return param key or null if no mode is provided ({@link CryptorMode#ALL}
     *         will be used)
     */
        ...
        do {
            ...
            try {
                ...
                if (isResponseValid(responseCode)) {
                    ...
                    // Decrypt if needed
                    // ONS:: Disabled for now.
                    WebserviceCryptor readCryptor = null; // getReadCryptor();
                    if (readCryptor != null) {
     ...
     * Note that this requires the body to be in the "standard" form, mostly used by
     * queries
     * That is {"status": "OK", "header": ..., "ts": ..., "body": {...}}
     * For webservice clients that use a different format, please use
     * {@link #getBasicJsonResponseBody}
     *
    ...
    protected HttpURLConnection buildConnection() throws Exception {
         ...
         * Post parameters
         * 
         * @see http://stackoverflow.com/questions/9767952/how-to-add-parameters-to-
         * httpurlconnection-using-post
         */
        ...
        /*
         * Once we write to the output stream, we cannot update anything in the
         * connection
         */
        if (postContent != null) {
    ...
    private byte[] buildPostParameters(PostDataProvider<?> provider) throws Exception {
         ...
         * ONS:: Disabled for now.
         */
        WebserviceCryptor cryptor = null; // getPostCryptor();
        if (cryptor != null) {
    ...
    protected void addRequestSignatures(HttpURLConnection connection, @Nullable byte[] body) {
        ...
        connection.setRequestProperty("X-ONS-Signature", signatureHeader.toString());
    }
    ...
    protected String getSignatureBody(HttpURLConnection connection, List<String> keys) {
        ...
        while (iterator.hasNext()) {
            ...
            if (!TextUtils.isEmpty(value)) {
                ...
                // If an property is null or empty, we don't include it at all and remove the
                // key from the list
                iterator.remove();
            }
        }
    ...
    /**
     * Return the {@link Parameters} key to search specific connect timeout for this
     * webservice
     *
    ...
    /**
     * Return the {@link Parameters} key to search specific read timeout for this
     * webservice
     *
     * @return key if we need specific value, null otherwise
     */
    ...
    /**
     * Return the {@link Parameters} key to search specific number of retry for this
     * webservice
     *
     * @return key if we need specific value, null otherwise
     */
    ...
    public static class WebserviceError extends Throwable {
        ...
        /**
         * Number of seconds we have to wait before sending another request to a
         * webservice who failed.
         *
         * Server can respond with an HTTP status code 429
         * ({@link Reason#TOO_MANY_REQUESTS})
