...
import com.ons.android.ONSNotificationChannelsManager;
...
public class NotificationAuthorizationStatus {
    ...
    public static void checkForNotificationAuthorizationChange(@NonNull Context context) {
        ...
        if (lastNotificationAuthorizationStatus != hasNotificationAuthorization) {
            ...
            boolean sdkReady = runtimeManager.runIfReady(() -> {
                ...
                ONSPushRegistration registration = PushModuleProvider.get().getRegistration(context);
    ...
    // Returns whether the app can potentially show a notification using ons
    ...
    public static boolean canAppShowNotifications(
        @NonNull Context context,
        @NonNull ONSNotificationChannelsManager channelsManager
    ) {
        ...
        return (
            areONSNotificationsEnabled(context) &&
    ...
    @VisibleForTesting
    static boolean areONSNotificationsEnabled(@NonNull Context context) {
        try {
            ...
            Logger.internal(TAG, "Error while getting ONS notification type", e);
        }
    ...
    static boolean isDefaultChannelEnabled(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            return canChannelShowNotifications(
                ...
                ONSNotificationChannelsManagerPrivateHelper.getChannelId(channelsManager),
                true
            );
        }
        return true;
    ...
    static boolean canChannelShowNotifications(
        ...
        if (channel == null && autoCreatedChannel) {
            // As ONS's channel is lazily created, it not existing will result in a notification
            // being shown
            return true;
        } else if (channel == null) {
            // The channel won't be created by ONS, so return false if it doesn't exist
            return false;
        }
