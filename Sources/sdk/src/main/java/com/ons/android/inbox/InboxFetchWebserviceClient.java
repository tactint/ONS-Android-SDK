...
import com.ons.android.ONSWebservice;
 ...
 */
public class InboxFetchWebserviceClient extends ONSWebservice implements TaskRunnable {
    ...
    public void run() {
        try {
            ...
            if (e1.getReason() == WebserviceError.Reason.FORBIDDEN) {
                ...
                listener.onFailure("Inbox API call error: ONS SDK has been globally Opted Out.");
            } else {
    ...
    protected static InboxNotificationContentInternal parseNotification(JSONObject json)
        ...
        try {
            ...
            final InternalPushData onsData = new InternalPushData(payload.getJSONObject("com.ons"));
            ...
            identifiers.additionalData = onsData.getExtraParameters();
            ...
            c.body = payload.reallyOptString(ONS.Push.BODY_KEY, null);
            c.title = payload.reallyOptString(ONS.Push.TITLE_KEY, null);
