...
import com.ons.android.di.providers.EmbeddedBannerContainerProvider;
import com.ons.android.di.providers.LocalBroadcastManagerProvider;
import com.ons.android.messaging.model.BannerMessage;
import com.ons.android.messaging.view.formats.EmbeddedBannerContainer;
import com.ons.android.module.MessagingModule;

/**
 * ONSBannerView handles operations related to a ONS In-App Messaging banner, when used in manual mode.
 * <p>
...
public class ONSBannerView {
     ...
     * If you'd like to attach the banner on a CoordinatorLayout, you should use {@link #show(View)}, or have your activity implement {@link ONS.Messaging.DisplayHintProvider}.
     * <br/>
     * This can only be called once per instance of ONSBannerView, even if {@link #dismiss(boolean)} has been called.
     * <br/>
    ...
    public void show(@NonNull Activity activity) {
        ...
        if (activity instanceof ONS.Messaging.DisplayHintProvider) {
            ONS.Messaging.DisplayHint hint =
                ((ONS.Messaging.DisplayHintProvider) activity).getONSMessageDisplayHint(rawMessage);
            if (hint != null) {
                if (hint.strategy == ONS.Messaging.DisplayHintStrategy.EMBED) {
                    if (hint.view instanceof FrameLayout) {
                        embed((FrameLayout) hint.view);
                    } else {
                        Logger.error(MessagingModule.TAG, "Could not embed ONSBannerView, internal error.");
                    }
                    return;
                }
        ...
        if (targetView != null) {
            ...
            Logger.error(
                MessagingModule.TAG,
                "Could not show ONSBannerView: the given activity doesn't seem to have a valid content view"
            );
        }
    }
     ...
     * ONSBannerView will automatically explore your view hierarchy to find the most appropriate view to display itself onto.
        ...
        new Handler(anchorView.getContext().getMainLooper())
            .post(() -> {
                try {
                    ...
                    Logger.error(
                        MessagingModule.TAG,
                        "Could not show ONSBannerView: internal error. Is your anchor view valid and part of the hierarchy?"
                    );
                }
            });
    }
     ...
     * <br/>
     * Do not make any assumption about the views that ONS will add, as it is merely an implementation detail and subject to change.
     *
        ...
        new Handler(embedLayout.getContext().getMainLooper())
            .post(() -> {
                try {
                    ...
                    Logger.error(
                        MessagingModule.TAG,
                        "Could not show ONSBannerView: internal error. Is your container layout valid and part of the hierarchy."
                    );
                }
            });
    }
