 ...
 */
@PublicSDK
public class ONSModalContent implements ONSInAppMessage.Content {
    ...
    ONSModalContent(@NonNull ModalMessage from) {
        ...
        if (from.globalTapAction != null) {
            globalTapAction = new ONSModalContent.Action(from.globalTapAction);
        }

        if (from.ctas != null) {
            for (com.ons.android.messaging.model.CTA fromCTA : from.ctas) {
                ctas.add(new ONSModalContent.CTA(fromCTA));
            }
        }
