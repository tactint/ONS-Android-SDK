...
import com.ons.android.ONSActionActivity;
import com.ons.android.ONSNotificationInterceptor;
import com.ons.android.ONSPermissionListener;
import com.ons.android.ONSPushHelper;
import com.ons.android.ONSPushNotificationPresenter;
import com.ons.android.ONSPushPayload;
import com.ons.android.ONSPushRegistration;
import com.ons.android.ONSPushService;
import com.ons.android.IntentParser;
import com.ons.android.PushNotificationType;
...
import com.ons.android.push.PushRegistrationProviderFactory;
...
@Module
@Singleton
public class PushModule extends ONSModule {
     ...
     */
    private ONSNotificationInterceptor notificationInterceptor = null;
     ...
     */
    public void setNotificationInterceptor(ONSNotificationInterceptor interceptor) {
     ...
     */
    public ONSNotificationInterceptor getNotificationInterceptor() {
        ...
        RuntimeManagerProvider
            ...
            .run(state -> {
                if (state == State.OFF) {
                    Logger.warning(
                        TAG,
                        "Call to dismissONSNotifications made while SDK is not started, please call this method only after ONS.onStart."
                    );
                    return;
                }
     ...
     */
    public boolean isONSPush(Intent intent) {
        if (intent == null) {
            return false;
        }
     ...
     * Firebase variant
     */
    public boolean isONSPush(RemoteMessage message) {
        if (message == null) {
            return false;
        }
     ...
     */
    @Nullable
    public ONSPushRegistration getRegistration() {
        AtomicReference<ONSPushRegistration> registration = new AtomicReference<>();
        RuntimeManagerProvider
            .get()
            .run(state -> {
     ...
     */
    public ONSPushRegistration getRegistration(@NonNull Context context) {
        try {
            ...
            return new ONSPushRegistration(registrationProvider, registrationID, senderID, gcpProjectID);
        } catch (Exception e) {
     ...
     */
    public void appendONSData(Intent pushIntent, Intent openIntent) {
        try {
            appendONSData(pushIntent.getExtras(), openIntent);
        } catch (Exception e) {
            Logger.internal(TAG, "Error while appending ons data to intent", e);
            Logger.error(TAG, "Error while appending ONS data to open intent : " + e.getLocalizedMessage());
        }
    }
     ...
     * @param openIntent       the intent of the notification the will be triggered when the user clicks on it
     */
    public void appendONSData(Bundle pushIntentExtras, Intent openIntent) {
        try {
            InternalPushData data = InternalPushData.getPushDataForReceiverBundle(pushIntentExtras);
            if (data == null) {
                Logger.error(
                    TAG,
                    "Error while appending ONS data to open intent : the pushIntentExtras seems to not be a ONS Push intent extras. Aborting"
                );
                return;
            }
     ...
     */
    public void appendONSData(@NonNull RemoteMessage remoteMessage, Intent openIntent) {
        Bundle extras = ONSPushHelper.firebaseMessageToReceiverBundle(remoteMessage);
        if (extras == null) {
            Logger.error(TAG, "Could not read data from Firebase message");
            return;
        }
        appendONSData(extras, openIntent);
    }
    ...
    public PendingIntent makePendingIntent(
        ...
        launchIntent.setAction("ons_" + Long.toString(System.currentTimeMillis()));
        ONS.Push.appendONSData(pushIntentExtras, launchIntent);
    ...
    public PendingIntent makePendingIntent(
        ...
        Bundle extras = ONSPushHelper.firebaseMessageToReceiverBundle(message);
        if (extras == null) {
            extras = new Bundle();
        }
     ...
     * <p>
     * This is useful for custom receivers, {@link com.ons.android.ONSNotificationInterceptor} or {@link com.ons.android.ONSDeeplinkInterceptor} implementations.
     *
    ...
    public PendingIntent makePendingIntentForDeeplink(
        ...
        final Intent launchIntent = new Intent(context, ONSActionActivity.class);
        launchIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        launchIntent.putExtra(ONSActionActivity.EXTRA_DEEPLINK_KEY, deeplink);
        launchIntent.setAction("ons_" + Long.toString(System.currentTimeMillis()));
    ...
    public boolean shouldDisplayPush(Context context, Intent intent) {
        try {
            return isManualDisplayModeActivated() && isONSPush(intent);
        } catch (Exception e) {
    ...
    public boolean shouldDisplayPush(Context context, RemoteMessage message) {
        try {
            return isManualDisplayModeActivated() && isONSPush(message);
        } catch (Exception e) {
            Logger.internal(TAG, "Error while evaluating if should display push", e);
            return true;
        }
    ...
    public void displayNotification(
        Context context,
        Intent intent,
        ONSNotificationInterceptor interceptor,
        ...
        try {
            ...
            ONSPushNotificationPresenter.presentNotification(
                context,
                intent.getExtras(),
                ONSPushPayload.payloadFromReceiverIntent(intent),
                interceptor
            );
        } catch (Exception e) {
     ...
     */
    public void displayNotification(Context context, RemoteMessage message, ONSNotificationInterceptor interceptor) {
        try {
            ...
            ONSPushNotificationPresenter.presentNotification(
                context,
                extras,
                ONSPushPayload.payloadFromReceiverExtras(extras),
                interceptor
            );
        } catch (Exception e) {
            ...
            .submit(
                new TaskRunnable() {
                    ...
                    public void run() {
                        ...
                        final ONSPushRegistration registration = new ONSPushRegistration(
                            provider.getShortname(),
                            registrationID,
    ...
    /**
     * Injects a registration ID into ONS, either from a builtin provider or the user's
     */
    private void emitRegistration(@NonNull Context context, @NonNull final ONSPushRegistration registration) {
        final Context appContext = context.getApplicationContext();

        // Broadcast a ONS-emitted registration intent
        final Intent i = new Intent(ONS.ACTION_REGISTRATION_IDENTIFIER_OBTAINED);
        i.putExtra(ONS.EXTRA_REGISTRATION_PROVIDER_NAME, registration.getProvider());
        i.putExtra(ONS.EXTRA_REGISTRATION_IDENTIFIER, registration.getToken());
        i.putExtra(ONS.EXTRA_REGISTRATION_SENDER_ID, registration.getSenderID());
        i.setPackage(appContext.getPackageName());
        appContext.sendBroadcast(i, ONS.getBroadcastPermissionName(appContext));
    ...
    public static boolean isONSPushServiceAvailable() {
        try {
            final PackageManager packageManager = RuntimeManagerProvider.get().getContext().getPackageManager();
            final Intent intent = new Intent(RuntimeManagerProvider.get().getContext(), ONSPushService.class);
    ...
    private void printRegistration(@NonNull ONSPushRegistration registration) {
     ...
     */
    public void requestNotificationPermission(@NonNull Context context, @Nullable ONSPermissionListener listener) {
        if (context == null) {
