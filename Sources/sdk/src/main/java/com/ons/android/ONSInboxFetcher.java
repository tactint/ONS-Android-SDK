...
import com.ons.android.inbox.InboxFetcherInternal;
import java.util.List;

/**
 * ONSInboxFetcher allows you to fetch notifications that have been sent to a user (or installation, more on that later) in their raw form,
 ...
 * <p>
 * Once you get your ONSInboxFetcher instance, you should call {@link #fetchNewNotifications(OnNewNotificationsFetchedListener)} to fetch the initial page of messages: nothing is done automatically.
 * This method is also useful to refresh the list.<br/>
 * <p>
 * In an effort to minimize network and memory usage, messages are fetched by page (onses of messages):
 ...
 * <p>
 * As ONSInboxFetcher caches answers from the server, instances of this class should be tied to the lifecycle of the UI consuming it (if applicable).<br/>
 * For example, you should keep a reference to this object during your Activity's entire life.<br/>
 * Another reason to keep the object around, is that you cannot mark a message as read with another ONSInbox instance that the one
 * that gave you the message in the first place.<br/>
 * <p>
 * A ONSInboxFetcher instance will hold to all fetched messages: be careful of how long you're keeping the instances around.<br/>
 * You can also set a upper messages limit, after which ONSInbox will stop fetching new messages, even if you call fetchNextPage.<br/>
 * <p>
 * Note: You will always be called back on the thread that you instantiated ONSInboxFetcher on. use {@link #setHandlerOverride(Handler)} if you want to change that behaviour.
 */
...
public class ONSInboxFetcher {
    ...
    ONSInboxFetcher(InboxFetcherInternal internal) {
        this.impl = internal;
    }
    ...
    /**
     * Maximum number of notifications to fetch. This allows you to let ONS manage the upper limit itself, so you can be sure not to use a crazy amount of memory.
     ...
     */
    public void markAsRead(ONSInboxNotificationContent notification) {
     ...
     */
    public void markAsDeleted(ONSInboxNotificationContent notification) {
     ...
     */
    @NonNull
    public List<ONSInboxNotificationContent> getFetchedNotifications() {
            ...
            listener =
                new OnNewNotificationsFetchedListener() {
                    ...
                    public void onFetchSuccess(
                        @NonNull final List<ONSInboxNotificationContent> notifications,
    ...
    public interface OnNextPageFetchedListener {
        void onFetchSuccess(@NonNull List<ONSInboxNotificationContent> notifications, boolean endReached);
