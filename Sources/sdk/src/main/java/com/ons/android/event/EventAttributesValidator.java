...
/**
 * Class that validates that a ONSEventAttributes object is valid
 */
public class EventAttributesValidator {
    ...
    public static List<String> computeValidationErrors(ONSEventAttributes eventData) {
    ...
    private static List<ValidationError> visitObject(ONSEventAttributes eventData, Breadcrumbs breadcrumbs) {
    ...
    private static List<ValidationError> visitAttributeValue(EventTypedAttribute attribute, Breadcrumbs breadcrumbs) {
        ...
        switch (attribute.type) {
            ...
            case STRING_ARRAY:
                    ...
                    if (baseArrayError != null) {
                        ...
                        if (attribute.type == AttributeType.OBJECT_ARRAY) {
                            try {
                                ...
                                List<ONSEventAttributes> objectArrayValue = (List<ONSEventAttributes>) anyArrayValue;
                ...
                break;
            case OBJECT:
                if (attribute.value instanceof ONSEventAttributes) {
                    mergeErrors(visitObject((ONSEventAttributes) attribute.value, breadcrumbs), errors);
                } else {
                    errors.add(genericTypecastError);
                }
                break;
    ...
    private static List<ValidationError> visitAttributeObjectArrayValue(
        List<ONSEventAttributes> array,
        Breadcrumbs breadcrumbs
    ) {
        List<ValidationError> errors = new ArrayList<>();
        for (int i = 0; i < array.size(); i++) {
            ONSEventAttributes value = array.get(i);
            Breadcrumbs itemBreadcrumbs = breadcrumbs.appending(i);
