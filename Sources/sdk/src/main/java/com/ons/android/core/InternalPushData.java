...
import com.ons.android.ONSNotificationAction;
...
import com.ons.android.json.JSONHelper;
...
public class InternalPushData {
    ...
    /**
     * Key to retrieve the silent boolean (if true, ONS shouldn't handle the notification)
     */
     ...
     */
    public static final String BATCH_BUNDLE_KEY = "com.ons";

    private String jsonPayload;
    private JSONObject payload;

    public InternalPushData(String onsData) {
        if (onsData == null || onsData.isEmpty()) {
            throw new NullPointerException("Cannot init PushData without the associated JSON data");
        }

        try {
            jsonPayload = onsData;
            payload = new JSONObject(onsData);
        } catch (JSONException e) {
            throw new IllegalArgumentException("Error while parsing JSON data", e);
        }
    }

    public InternalPushData(JSONObject onsData) {
        if (onsData == null || onsData.keySet().isEmpty()) {
            throw new NullPointerException("Cannot init PushData without the associated JSON data");
        }

        jsonPayload = onsData.toString();
        payload = onsData;
    }
    ...
    public static InternalPushData getPushDataForReceiverBundle(@Nullable Bundle extras) {
        if (extras != null && !extras.isEmpty()) {
            String onsData = extras.getString(BATCH_BUNDLE_KEY);
            if (onsData != null) {
                return new InternalPushData(onsData);
            }
        }
    ...
    public static InternalPushData getPushDataForFirebaseMessage(@Nullable RemoteMessage message) {
        ...
        if (data != null && data.size() > 0) {
            String onsData = data.get(BATCH_BUNDLE_KEY);
            if (onsData != null) {
                return new InternalPushData(onsData);
            }
        }
    ...
    public List<ONSNotificationAction> getActions() {
        final List<ONSNotificationAction> retVal = new ArrayList<>();
        ...
        if (actions != null) {
            for (int i = 0; i < actions.length(); i++) {
                ...
                final ONSNotificationAction tmpAction = new ONSNotificationAction();
    ...
    public ONSNotificationSource getSource() {
        ...
        if ("c".equalsIgnoreCase(source)) {
            return ONSNotificationSource.CAMPAIGN;
        } else if ("t".equalsIgnoreCase(source)) {
            return ONSNotificationSource.TRANSACTIONAL;
        } else if ("tc".equalsIgnoreCase(source)) {
            return ONSNotificationSource.TRIGGER;
        }
        return ONSNotificationSource.UNKNOWN;
    }
