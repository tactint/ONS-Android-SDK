...
import com.ons.android.messaging.WebViewActionListener
...
@SmallTest
class ONSMessagingWebViewJavascriptBridgeTest {

    val context: Context = ApplicationProvider.getApplicationContext<Context>()

    @Test
    fun testPostMessage() {
        val bridge = ONSMessagingWebViewJavascriptBridge(context, null, null)
    ...
    fun testOpenDeeplink() {
        ...
        val bridge = ONSMessagingWebViewJavascriptBridge(context, null, mockActionListener)

        assertBridgeError(bridge.postMessage("openDeeplink", "{}"))
        assertBridgeResult("ok", bridge.postMessage("openDeeplink", """{"url": "https://pfs.gdn"}"""))
        assertBridgeResult("ok", bridge.postMessage("openDeeplink", """{"url": "https://pfs.gdn", "openInApp": true}"""))
        assertBridgeResult("ok", bridge.postMessage("openDeeplink", """{"url": "https://pfs.gdn", "openInApp": false}"""))
        assertBridgeResult("ok", bridge.postMessage("openDeeplink", """{"url": "https://pfs.gdn", "openInApp": true, "analyticsID": 2}"""))
        assertBridgeResult("ok", bridge.postMessage("openDeeplink", """{"url": "https://pfs.gdn", "openInApp": true, "analyticsID": "test_analytics"}"""))

        verify(mockActionListener).onOpenDeeplinkAction(Mockito.eq("https://pfs.gdn"), Mockito.eq(null), Mockito.eq(null))
        verify(mockActionListener).onOpenDeeplinkAction(Mockito.eq("https://pfs.gdn"), Mockito.eq(true), Mockito.eq(null))
        verify(mockActionListener).onOpenDeeplinkAction(Mockito.eq("https://pfs.gdn"), Mockito.eq(false), Mockito.eq(null))
        verify(mockActionListener).onOpenDeeplinkAction(Mockito.eq("https://pfs.gdn"), Mockito.eq(true), Mockito.eq(null))
        verify(mockActionListener).onOpenDeeplinkAction(Mockito.eq("https://pfs.gdn"), Mockito.eq(true), Mockito.eq("test_analytics"))
    }
    ...
    fun testPerformAction() {
        ...
        assertBridgeResult("ok", bridge.postMessage("performAction", """{"name":"ons.test", "args": {"arg1":"value"}}"""))
        assertBridgeResult("ok", bridge.postMessage("performAction", """{"name":"ons.test", "args": {"arg1":"value"}, "analyticsID": 2}"""))
        assertBridgeResult("ok", bridge.postMessage("performAction", """{"name":"ons.test", "args": {"arg1":"value"}, "analyticsID": " "}"""))
        assertBridgeResult("ok", bridge.postMessage("performAction", """{"name":"ons.test", "args": {"arg1":"value"}, "analyticsID": "test_analytics"}"""))

        verify(mockActionListener).onPerformAction(Mockito.eq("ons.test"), JSONObjectMockitoMatcher.eq(JSONObject().apply {
            put("arg1", "value")
        }), Mockito.eq(null))
...
class MockingBridge(context: Context, message: ONSMessage?, actionListener: WebViewActionListener?) : ONSMessagingWebViewJavascriptBridge(context, message, actionListener) {
    object Expectations {
    ...
    var onsConfigAllowsAdvertisingID = true
...
class MockMessage(val payload: JSONObject) : ONSMessage() {
