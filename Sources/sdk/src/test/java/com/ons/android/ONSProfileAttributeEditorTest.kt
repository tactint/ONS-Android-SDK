...
@SmallTest
class ONSProfileAttributeEditorTest : DITest() {

    private lateinit var trackerModule : TrackerModule

    override fun setUp() {
        super.setUp()
        simulateONSStart(ApplicationProvider.getApplicationContext())
        trackerModule = DITestUtils.mockSingletonDependency(TrackerModule::class.java, null)
    }

    @Test
    fun testFullMethods() {
        ONS.Profile.identify("arnaudr")
        ONSProfileAttributeEditor().apply {
            setLanguage("fr")
            setRegion("FR")
            setEmailAddress("test@pfs.gdn")
            setEmailMarketingSubscription(ONSEmailSubscriptionState.SUBSCRIBED)
        ...
        val expectedProfileDataChangedParams = JSONObject().apply {
            ...
            put("custom_attributes", JSONObject().apply {
                ...
                put("url_att.u", "https://pfs.gdn/pricing")
